...
import config
from AaruXMusix import Carbon, YouTube, app
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await AaruXMusix.force_stop_stream(chat_id)
...
db[chat_id][0]["markup"] = "stream"
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await AaruXMusix.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await AaruXMusix.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
